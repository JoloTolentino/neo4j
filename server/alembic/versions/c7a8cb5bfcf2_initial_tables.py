...
def upgrade() -> None:
    ...
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('uuid', sa.String(), nullable=False),
    sa.ForeignKeyConstraint(['movie_id'], ['movies.uuid'], ),
    sa.PrimaryKeyConstraint('uuid')
    )
    op.create_table('movie_tags',
    sa.Column('movie_title', sa.String(), nullable=False),
    sa.Column('tag', sa.Enum('horror', 'love', 'comedy', 'action', name='movietagenums'), nullable=False),
    sa.Column('uuid', sa.String(), nullable=False),
...
def downgrade() -> None:
    ...
    sa.Column('firstname', sa.VARCHAR(length=20), autoincrement=False, nullable=False),
    ...
    sa.Column('last_login', sa.VARCHAR(length=20), autoincrement=False, nullable=True),
    ...
    sa.Column('tier', postgresql.ENUM('FREE', 'PRO', 'ENTERPRISE', name='tiers'), autoincrement=False, nullable=False),
    sa.Column('expiration', sa.VARCHAR(length=40), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['uuid'], ['users.uuid'], name='subscriptions_uuid_fkey'),
    ...
    sa.Column('banned_at', sa.VARCHAR(length=50), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['uuid'], ['users.uuid'], name='banned_users_uuid_fkey'),
    ...
    sa.Column('lockout_until', sa.VARCHAR(length=30), autoincrement=False, nullable=True),
